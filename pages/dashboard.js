...
export default function Dashboard() {
  ...
  async function load() {
    try {
      ...
      const member = _meta?.isMember ?? false;
      setMember(member);
      isMemberRef.current = member;
  ...
  const save = useCallback(async (nextCfg) => {
    setSave('saving');
    try {
      // Memberでない場合はgatedキー（tags/blacklist/friends）を除外してPOST
      const payload = { ...(nextCfg || cfg) };
      if (!isMemberRef.current) {
        ROLE_GATED_KEYS.forEach(k => delete payload[k]);
      }
